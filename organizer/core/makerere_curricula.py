...
def name_for_code(code):
    """Looks up a course unit's real name from its code, e.g. "CSC2100" ->
    "Data Structures and Algorithms", by scanning every programme this
    module covers. Returns None if the code isn't in any of them (a
    manually-typed subject, or a Makerere programme not yet researched
    here) -- callers fall back to the bare code in that case."""
    global _CODE_NAME_CACHE
    if _CODE_NAME_CACHE is None:
        cache = {}
        for programme in CURRICULA.values():
            for year in programme["years"].values():
                for units in year.values():
                    for entry in units:
                        entry_code, sep, name = entry.partition(" - ")
                        if sep and entry_code and entry_code not in cache:
                            cache[entry_code] = name
        _CODE_NAME_CACHE = cache
    return _CODE_NAME_CACHE.get(code)
